 ...
 * 順序：WooCommerce → Powerhouse → Power Docs
 */
function _power_docs_manually_load_plugin(): void {
	// 1. 載入 WooCommerce
	...
	// 3. 載入 Power Docs plugin
	require dirname( __DIR__ ) . '/plugin.php';
}

tests_add_filter( 'muplugins_loaded', '_power_docs_manually_load_plugin' );
 ...
 */
function _power_docs_force_bootstrap(): void {
	if ( class_exists( 'J7\PowerDocs\Bootstrap' ) ) {
		\J7\PowerDocs\Bootstrap::instance();
	}
}

tests_add_filter( 'plugins_loaded', '_power_docs_force_bootstrap', 20 );

/**
 * 在 WordPress after_setup_theme 後建立 Powerhouse 自訂資料表
 * Power Docs 使用 ph_access_itemmeta 儲存知識庫授權資訊
 */
function _power_docs_create_powerhouse_tables(): void {
	if ( class_exists( 'J7\Powerhouse\Domains\Limit\Utils\CreateTable' ) ) {
		try {
			...
			echo "警告：建立 Powerhouse 自訂資料表失敗：{$th->getMessage()}\n";
		}
	}
}

tests_add_filter( 'after_setup_theme', '_power_docs_create_powerhouse_tables' );
